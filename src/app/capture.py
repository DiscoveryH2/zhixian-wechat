...
def find_wechat_hwnd():
    ...
    u32.EnumWindows.argtypes = [type(cb), ctypes.c_void_p]
...
class Capture:
    ...
    def on_frame_arrived(self, frame, control):
        ...
        with self._lock:
            self._latest, self._latest_at = full, time.monotonic()
        if full.max() == 0:
            return
        ...
        # Include the header: switching between conversations with identical text
        # must still invalidate the active session identity.
        chat = full[self.area[5]:y1, x0:x1]
    ...
    def latest(self, max_age=2.0):
        with self._lock:
            if self._latest is None or time.monotonic() - self._latest_at > max_age:
                return None
            return self._latest.copy(), self._latest_at
