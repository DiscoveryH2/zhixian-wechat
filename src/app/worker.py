...
def run(q, hwnd, enabled, stopped=None):
            ...
            if full is not None:
                ...
                if area is None:
                    ...
                    if head is None or not np.array_equal(crop, head):  # 名字没动就别白跑一次 OCR
                        ...
                        # Unknown titles invalidate the target immediately.
                        # Never alias similar names or reuse a previous chat.
                        if not name:
                            title = ""
                            q.put(("status", "会话标题无法识别，已停止读取本帧"))
                            continue
                        if name != title:
                            title = name
                            q.put(("chat", title))
