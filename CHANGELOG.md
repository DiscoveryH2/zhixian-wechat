...
- 新增独立的「知弦 Agent」会话巡检页。用户从会话目录明确选择最多三个会话，Agent 按需读取各会话最近一页，在有界工具链中区分即时回复、稍后跟进、等待与无需动作；只有判断足够明确时才尝试生成候选。
- 展示会话来源、真实消息 ID 对应的证据片段和每步工具运行记录；媒体占位或证据不足时保留待复核状态，不把标签当成已理解的内容。Agent 本身没有微信发送工具，复制候选仍由用户决定是否使用。
- Agent 在隔离模型进程中运行，不持久化本次巡检结果；限制会话数、消息数和工具种类，并以合成会话测试协议、边界与隐私。自动回复原有白名单和发送核验不变。
- 合成端到端桌面桥接测试与使用合成文字的真实 OpenRouter 协议冒烟已通过；这不代表真实微信消息发送验收。

## 1.5.3 预览版

- 修复托盘中的微信窗口画面静止时，标题 OCR 处理耗时把发送核验推过 1 秒新鲜度上限，导致发送键调用前错误阻止的问题。知弦在静止画面过期时会重建一次窗口采集，并在读取标题前核对画面时间。
- 数据库自动回复的发送失败现在用固定、无聊天正文的原因码写入脱敏审计，并在自动回复页显示具体核验失败原因；发送结果不明仍暂停，不自动重试。
- 本机已确认此前失败只记录了 `blocked`，未调用发送键；新版只读预检可到达空输入框核验。真实正向发出仍等待下一条通过完整安全门控的新消息。

## 1.5.2 预览版

- 修复从窗口 OCR 切换到本机数据库后，旧自动回复名单仍带 OCR 会话 ID，导致点击启动时被旧来源校验拒绝的问题。只有名称和单聊/群聊类型在数据库目录里唯一匹配时才迁移，否则要求重新选择。
- 修复应用重启后数据库白名单未先加载到会话目录时无法再次保存或启动；现在会用稳定会话 ID 从本机数据库重新核验。每次启动后的自动发送开关仍需用户在界面明确确认。

## 1.5.1 预览版

- 微信收至托盘或最小化时，数据库仍可读取新消息；发送时若目标群本来已在微信打开，知弦临时恢复窗口核验标题和输入框，完成后恢复原窗口状态。不会按名称盲目切换会话。
- 从 CipherTalk 当前账号配置读取自己的显示名；群聊数据库消息只有明确 `@` 该称呼时才标记为指向自己，默认「仅回复明确 @ 我」规则因此可用于数据库来源。
- 持续监听分页覆盖会话索引；数据库来源的自动回复不再因为其他会话更新而被当前会话指针误拦截。明确 @ 的群消息按正常回复置信度判定，未明确指向用户的群消息继续使用更高门槛。
- 真机后台读取与发送判断结果见 [1.5.1 发行说明](docs/releases/1.5.1.md)。
